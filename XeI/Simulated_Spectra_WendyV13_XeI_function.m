...
function [EEE,SSS] = Simulated_Spectra_WendyV13_XeI_function(b_B,beta_B,De_l,beta_l,Re_l,De_r,beta_r,Re_r,a_off_l,Rs_off_l,a_on_l,Rs_on_l,a_off_r,Rs_off_r,a_on_r,Rs_on_r)
...
% % % % %calcualted from baseline ratio
% % % % %  a1 =      0.4406  ;% (0.3908, 0.4903)
% % % % %        b1 =        3.73 ;%  (3.709, 3.75)
% % % % %        d =       8.941 ;%  (-16.95, 34.83)
% % % % %        c =       4.383 ;%  (3.448, 5.318)
% % % % %        a2 =      0.1057 ;%   (-0.06883, 0.2802)
% % % % %        b2 =       4.126 ;%  (3.978, 4.274)
% % % %  %
% % % %  mur = @(R)  a1^2./((a1^2+(R-b1).^2).*(1+exp(d*(R-c))))+a2^2./((a2^2+(R-b2).^2).*(1+exp(-d*(R-c))));
n=7; 
p=[-91.1736973326590,2389.04295253899,-26782.1722831967,166509.059708436,-620044.328380169,1382926.15951339,-1710569.26072144,905191.356717507];
mur = @(x)  p(1)*x.^n+p(2)*x.^(n-1)+p(3)*x.^(n-2)+p(4)*x.^(n-3)+p(5)*x.^(n-4)+p(6)*x.^(n-5)+p(7)*x.^(n-6)+p(8)*x.^(n-7);

% n=6;
%  p  =[-20.1091500693184,457.851939929672,-4334.02190130506,21832.5554988222,-61731.2072029696,92895.5653798124,-58129.4048803521];
% mur = @(x)  p(1)*x.^n+p(2)*x.^(n-1)+p(3)*x.^(n-2)+p(4)*x.^(n-3)+p(5)*x.^(n-4)+p(6)*x.^(n-5)+p(7)*x.^(n-6);

% n=5;
% p=[3.26069781717114,-60.2473258690403,444.775662245493,-1641.79033243667,3033.57604502890,-2246.12611078591];
%  mur = @(x)  p(1)*x.^n+p(2)*x.^(n-1)+p(3)*x.^(n-2)+p(4)*x.^(n-3)+p(5)*x.^(n-4)+p(6)*x.^(n-5);

%   n=3;
% p = [0.773296225797535,-10.4178204624419,45.2084351228290,-62.8265592491729];
%  mur = @(x)  p(1)*x.^n+p(2)*x.^(n-1)+p(3)*x.^(n-2)+p(4)*x.^(n-3);
murpts = mur(R);
%%%%%%%%%%%%%%%%%%%%%
% %  constant mur
% murpts = 1;
