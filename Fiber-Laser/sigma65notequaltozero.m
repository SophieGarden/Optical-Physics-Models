...
r2=600e-6/2;%inner cladding radius=Diameter-short axis of D shape inner cladding/2
r3=840e-6/2;%m. polymer,outer cladding radius
...
%----------高掺杂-----------
Ner=4e25;  %4e25; %%掺杂浓度Er  但是实际参与的粒子数可能没有那么多！！！
Nyb=6e26; %5e26;  %%掺杂浓度Yb (Nyb/Ner=14) the ratio of Nyb/Ner is approximately 4~20/8~30

C2=3.5e-24+(Ner-4.4e25)*2.4107e-49;%能量上转换系数 Ner>4.4e25的情况下
R61=1e-22+(Nyb-1e25)*4.00e-49;%energy transfer coefficient适用于  1e25<Nyb<1e27  
...
lams1=1.550e-6;%信号光波长1
lams2=1.064e-6;%信号光波长2
lamp=0.975e-6;%泵浦波长
...
%=============================================================================
L=6; %length of fiber
m=60; %Max 步数 ----L/m 必须等于有限小数，否则提示方程不相等 %与下面的M()数组区别
...
if u==1  %u==1的时候要从yb没出光时候开始，因为此时估算eta=0.4,对yb出光后不适用
 eta(1,u)=0.5; %η,泵浦光转化成热的效率，也就是未被吸收的泵浦光比率
else
...
Sgm65(g,u)=sgm65r*Zf(6,1)/Zf(6,2); %E(6,1)->E(5,3)
%Sgm56(g,u)=0; %Attention！！！因为sgm56<<sgm65,则可以并且一定要令sgm56=0;否则会出现Ts2*(n6*sgm65-n5*sgm56)-alpha2 < 0的情况，则p(3,:)不增大反而减小！！！！ 
Sgm56(g,u)=Sgm65(g,u)*Zf(6,2)/Zf(5,2)*exp((E(6,1)-E(5,1)-(E(6,1)-E(5,3)))*M(2)); 
